...
public:
    ...
    static bool RemoveTaskRunner(const std::string& taskName);
    ...
    ~TaskRunner();
private:
    explicit TaskRunner(napi_env env);

    static std::unordered_map<std::string, std::unique_ptr<TaskRunner>> runners;
    static std::mutex runnersMutex_;

    napi_threadsafe_function tsfn_ = nullptr;

    static void CallJs(napi_env env, napi_value js_cb, void* context, void* data);
    static void Finalize(napi_env env, void* raw, void* hint);
};
