...
public:
    ...
    static void DestroyTaskRunner(const std::string& name);
    ...
    static std::shared_ptr<JSFunction> GetJSFunctionRef(const std::string& name);

    static size_t CleanupInvalidSessions();
private:
};
...
#define JSBIND_FUNCTION(__name, ...) \
    aki::FunctionDefiner JSBIND_UNIQUE(definer, __LINE__)(aki::AliasName(#__name, ##__VA_ARGS__), &__name)

#define JSBIND_PFUNCTION(__name, ...) \
    aki::PFunctionDefiner JSBIND_UNIQUE(definer, __LINE__)(aki::AliasName(#__name, ##__VA_ARGS__), &__name)

#define JSBIND_GLOBAL() namespace

#define JSBIND_SCOPED_FUNCTION(__scoped, __name, ...) \
    aki::FunctionDefiner JSBIND_UNIQUE(definer, __LINE__)(__scoped, aki::AliasName(#__name, ##__VA_ARGS__), &__name)
...
#define JSBIND_FIELD(__name, __getter, __setter) \
    (definer.AddField(__name, &C::__getter, &C::__setter))
