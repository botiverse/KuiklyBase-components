/*
 * Copyright (C) 2022 Huawei Device Co., Ltd.
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef AKI_CLASS_WRAPPER_H
#define AKI_CLASS_WRAPPER_H

#include <memory>
#include <unordered_map>
#include <node_api.h>

#include "aki/config.h"
#include "aki/type/type_flag.h"
#include "aki/logging/logging.h"

namespace aki {

class AKI_EXPORT ClassWrapperBase {
public:
    ClassWrapperBase(void* obj, TypeFlag flag);
    ~ClassWrapperBase() = default;

    void Invalidate() { unsafeObj_ = nullptr; }

    bool IsValid() const { return unsafeObj_ != nullptr; }

protected:
    void* unsafeObj_;
    TypeFlag flag_;
};

template <class C>
class ClassWrapper : public ClassWrapperBase {
public:
    ClassWrapper(C *instance, TypeFlag flag)
        : ClassWrapperBase(instance, flag) { }
    ClassWrapper(std::shared_ptr<C> instance, TypeFlag flag)
        : ClassWrapperBase(instance.get(), flag), nativeObj_(instance) { }

    ~ClassWrapper()
    {
        AKI_DLOG(DEBUG) << "~ClassWrapper unsafe obj: " << unsafeObj_;
        if (flag_ == TypeFlag::IS_SHARED_PTR) {
            AKI_DLOG(DEBUG) << "~ClassWrapper SHARED_PTR use count : " << nativeObj_.use_count();
        }
    }

    template<typename... Args>
    static void ConstructorWrapper(napi_env env, napi_value jsThis, Args&&... args)
    {
        (new ClassWrapper(std::make_shared<C>(std::forward<Args>(args)...),
                          TypeFlag::IS_SHARED_PTR
        ))->WrapThis(env, jsThis);

        return;
    }

    static napi_value ExternalWrapper(napi_env env, napi_callback_info info)
    {
        napi_status status;
        napi_value jsThis;
        size_t argc = 2;
        napi_value args[2];
        status = napi_get_cb_info(env, info, &argc, args, &jsThis, nullptr);
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_cb_info failed in ClassWrapper::ExternalWrapper");
            return nullptr;
        }

        C* instance = nullptr;
        status = napi_get_value_external(env, args[0], ((void**)&instance));
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_value_external failed in ClassWrapper::ExternalWrapper");
            return nullptr;
        }

        uint32_t result;
        status = napi_get_value_uint32(env, args[1], &result);
        TypeFlag flag = static_cast<TypeFlag>(result);

        ClassWrapper* wrapperObj = nullptr;
        if (flag == TypeFlag::IS_SHARED_PTR) {
            std::shared_ptr<C> sharedPtr;
            sharedPtr.reset(instance);
            wrapperObj = new ClassWrapper(sharedPtr, flag);
        } else {
            wrapperObj = new ClassWrapper(instance, flag);
        }
        wrapperObj->WrapThis(env, jsThis);

        return jsThis;
    }

    void WrapThis(napi_env env, napi_value jsThis)
    {
        napi_status status;
        void* finalizeHint = reinterpret_cast<void*>(static_cast<uintptr_t>(flag_));
        status = napi_wrap(env,
                           jsThis,
                           this,
                           DestroyInstance,
                           finalizeHint,
                           nullptr);
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            AKI_DLOG(ERROR) << "napi_wrap failed in ClassWrapper::WrapThis, potential memory leak";
            delete this;
            return;
        }

        AKI_DLOG(DEBUG) << "ClassWrapperBase wrap obj: " << this;
    }

    static C* UnWrapper(napi_env env, napi_value jsThis)
    {
        napi_status status;
        napi_valuetype type;
        status = napi_typeof(env, jsThis, &type);
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_typeof failed in ClassWrapper::UnWrapper");
            return nullptr;
        }
        if (type == napi_null) {
            return nullptr;
        }

        ClassWrapper* wrapperObj = nullptr;
        status = napi_unwrap(env, jsThis, reinterpret_cast<void**>(&wrapperObj));
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_unwrap failed in ClassWrapper::UnWrapper");
            return nullptr;
        }
        if (wrapperObj == nullptr) {
            napi_throw_error(env, nullptr, "wrapperObj is nullptr after napi_unwrap");
            return nullptr;
        }
        AKI_DLOG(DEBUG) << "ClassWrapperBase unwrap obj: " << wrapperObj->unsafeObj_;

        if (!wrapperObj->IsValid()) {
            AKI_LOG(ERROR)
                << "ClassWrapper unwrapped object has been invalidated "
                << "(underlying C++ object may have been deleted)";
            return nullptr;
        }

        return reinterpret_cast<C*>(wrapperObj->unsafeObj_);
    }

    static std::shared_ptr<C>* GetShared(napi_env env, napi_value jsThis)
    {
        static std::shared_ptr<C> emptyPtr;
        napi_status status;
        napi_valuetype type;
        status = napi_typeof(env, jsThis, &type);
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_typeof failed in ClassWrapper::GetShared");
            return &emptyPtr;
        }
        if (type == napi_null) {
            return &emptyPtr;
        }
        
        ClassWrapper* wrapperObj = nullptr;
        status = napi_unwrap(env, jsThis, reinterpret_cast<void**>(&wrapperObj));
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_unwrap failed in ClassWrapper::GetShared");
            return &emptyPtr;
        }
        if (wrapperObj == nullptr) {
            napi_throw_error(env, nullptr, "wrapperObj is nullptr after napi_unwrap in GetShared");
            return &emptyPtr;
        }

        if (!wrapperObj->IsValid()) {
            AKI_LOG(ERROR) << "ClassWrapper unwrapped object has been invalidated";
            return &emptyPtr;
        }

        return &(wrapperObj->nativeObj_);
    }

private:
    static void DestroyInstance(napi_env env, void* nativeInstance, void* finalize_hint)
    {
        AKI_DLOG(DEBUG) << "ClassWrapperBase destroy obj: " << nativeInstance;
        ClassWrapper* wrapper = reinterpret_cast<ClassWrapper*>(nativeInstance);

        TypeFlag flag = static_cast<TypeFlag>(reinterpret_cast<uintptr_t>(finalize_hint));
        if (flag == TypeFlag::IS_POINTER) {
            if (wrapper != nullptr && wrapper->unsafeObj_ != nullptr) {
                AKI_LOG(ERROR)
                    << "IS_POINTER ClassWrapper destroyed with non-null unsafeObj_: "
                    << wrapper->unsafeObj_
                    << "Invalidate() was not called before C++ object deletion."
                    << " This may cause use-after-free if JS still holds a reference.";
                wrapper->unsafeObj_ = nullptr;
            }
        }

        delete wrapper;
    }

    std::shared_ptr<C> nativeObj_;
};
} // namespace aki
#endif //AKI_CLASS_WRAPPER_H
