...
public:
    ...
    // | static |
    static const InvokerInfoType& GetInvoker(int32_t id)
    {
        return GetInstance().invokers_[id];
    }

    // | static |
    static int32_t AddInvoker(I invoker)
    {
...
private:
    /// 所有Native函数类型为：R(*)(P...)的函数都由同一个Binder转发，
    /// 因此需要用Invoker Id区分，并且考虑索引性能，使用数组存储Invoker对象。
