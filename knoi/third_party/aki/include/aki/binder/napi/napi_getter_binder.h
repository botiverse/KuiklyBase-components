...
private:
    static napi_value SafetyWrapper(napi_env& env, napi_callback_info info)
    {
        ...
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_cb_info failed in NapiGetterBinder::SafetyWrapper");
            return nullptr;
        }
        AKI_DCHECK(bindInfo != nullptr);
        if (bindInfo == nullptr) {
            napi_throw_error(env, nullptr, "bindInfo is nullptr in NapiGetterBinder::SafetyWrapper");
            return nullptr;
        }
