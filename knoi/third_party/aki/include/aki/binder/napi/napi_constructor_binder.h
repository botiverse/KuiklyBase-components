...
public:
    ...
    static napi_value SafetyWrapper(napi_env& env, napi_callback_info info, std::index_sequence<Index...>)
    {
        ...
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_new_target failed in ConstructorBinder::SafetyWrapper");
            return nullptr;
        }
        if (target != nullptr) {
            // 以构造函数形式调用：new Class(...)
            ...
            AKI_DCHECK(status == napi_ok);
            if (status != napi_ok) {
                napi_throw_error(env, nullptr, "napi_get_cb_info failed in ConstructorBinder::SafetyWrapper");
                return nullptr;
            }

            auto tuple = std::make_tuple(
                NapiValueMaker<typename ValueDefiner<P>::RawType>(env, args[Index])...);
            auto valueTuple = std::make_tuple(
                NapiValueTrait<typename ValueDefiner<P>::RawType>::Cast(
                    std::get<Index>(tuple))...);
            ...
            napi_throw_error(env,
                "JSBind Error: Class constructor cannot be invoked without 'new'",
                "JSBind Error: Class constructor cannot be invoked without 'new'");
            return nullptr;
        }
    }
...
private:
    template<size_t... Index>
    static void CreateValueInner(napi_env env, napi_callback_info info,
                                 ArgStorage& storage, std::index_sequence<Index...>)
    {
        napi_status status;
        ...
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            AKI_DLOG(ERROR) << "napi_get_cb_info failed in ConstructorBinder::CreateValueInner";
            return;
        }
