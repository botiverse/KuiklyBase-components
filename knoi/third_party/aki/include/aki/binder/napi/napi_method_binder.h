...
private:
    ...
    static napi_value SafetyWrapper(napi_env env, napi_callback_info info, std::index_sequence<Index...>)
    {
        ...
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_cb_info failed in NapiMethodBinder::SafetyWrapper");
            return nullptr;
        }
        AKI_DCHECK(bindInfo != nullptr);
        if (bindInfo == nullptr) {
            napi_throw_error(env, nullptr, "bindInfo is nullptr in NapiMethodBinder::SafetyWrapper");
            return nullptr;
        }
        auto tuple = std::make_tuple(NapiValueMaker<typename ValueDefiner<P>::RawType>(env, args[Index])...);
        std::array<internal::Value*, sizeof...(P)> result = {
            static_cast<internal::Value*>(std::get<Index>(tuple))... };
...
#else
    template<size_t... Index>
    static napi_value WrapperForward(napi_env& env,
        ...
        std::index_sequence<Index...>)
    {
        if constexpr (std::is_void<R>::value) {
            ParentType::InnerWrapper(receiver, bindInfo,
                NapiValueTrait<P>::Cast(*values[Index])...);
            return nullptr;
        } else {
            return NapiValue<R>::ToNapiValue(env,
                ParentType::InnerWrapper(receiver, bindInfo,
                    NapiValueTrait<P>::Cast(*values[Index])...));
        }
    }
