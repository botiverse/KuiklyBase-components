...
private:
    ...
    static napi_value SafetyWrapper(napi_env env, napi_callback_info info, std::index_sequence<Index...>)
    {
        ...
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_cb_info failed in NapiPMethodBinder::SafetyWrapper");
            return nullptr;
        }
        AKI_DCHECK(bindInfo != nullptr);
        if (bindInfo == nullptr) {
            napi_throw_error(env, nullptr, "bindInfo is nullptr in NapiPMethodBinder::SafetyWrapper");
            return nullptr;
        }
        ...
        /// 获取ClassType对象实例
        std::shared_ptr<C> receiver;
        std::shared_ptr<C>* sharedPtr = ClassWrapper<C>::GetShared(env, jsThis);
        if (sharedPtr && *sharedPtr) {
            receiver = *sharedPtr;
        } else {
            C* rawPtr = ClassWrapper<C>::UnWrapper(env, jsThis);
            if (rawPtr == nullptr) {
                napi_throw_error(env, nullptr, "ClassWrapper unwrapped object is null");
                return nullptr;
            }
            receiver = std::shared_ptr<C>(rawPtr, [](C*) {
            });
        }
    ...
    static napi_value AsyncForward(napi_env env, InvokeWorkInfo* workData)
    {
        ...
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_create_promise failed in NapiPMethodBinder::AsyncForward");
            delete workData;
            return nullptr;
        }
        ...
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_create_string_utf8 failed in NapiPMethodBinder::AsyncForward");
            delete workData;
            return nullptr;
        }
        ...
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_create_async_work failed in NapiPMethodBinder::AsyncForward");
            delete workData;
            return nullptr;
        }
        ...
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_queue_async_work failed in NapiPMethodBinder::AsyncForward");
            delete workData;
            return nullptr;
        }
        return promise;
    ...
    static napi_value AsyncInvokeDoneHelper(napi_env env, napi_status status, std::false_type, void* data)
    {
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "async work failed in NapiPMethodBinder::AsyncInvokeDoneHelper");
            return nullptr;
        }
        AKI_DCHECK(data != nullptr);
        struct InvokeWorkInfo *arg = (struct InvokeWorkInfo *)data;
    ...
    static void AsyncInvokeDone(napi_env env, napi_status status, void* data)
    {
        ...
        AKI_DCHECK(status == napi_ok);
        if (status != napi_ok) {
            AKI_DLOG(ERROR) << "napi_delete_async_work failed in NapiPMethodBinder::AsyncInvokeDone";
        }
        arg->deferred = nullptr;
        delete arg;
    }
