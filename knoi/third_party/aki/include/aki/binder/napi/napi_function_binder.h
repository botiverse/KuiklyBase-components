...
private:
    ...
    static napi_value SafetyWrapper(napi_env& env, napi_callback_info info, std::index_sequence<Index...>)
    {
        ...
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_get_cb_info failed in SafetyWrapper");
            return nullptr;
        }
        AKI_DCHECK(bindInfo != nullptr);
        if (bindInfo == nullptr) {
            napi_throw_error(env, nullptr, "bindInfo is nullptr in SafetyWrapper");
            return nullptr;
        }
        ...
        std::index_sequence<Index...>)
    {
        ...
        if constexpr (std::is_void<R>::value) {
            ParentType::InnerWrapper(bindInfo,
                NapiValueTrait<P>::Cast(*values[Index])...);
            return nullptr;
        } else {
