...
int JSBind::bindFunction(const std::string& name, JSFunction func)
{
    if (name.empty()) {
        ...
        napi_throw_error(env, nullptr, "JSBind: bindFunction name cannot be empty");
        return -1;
    }
...
int JSBind::unbindFunction(const std::string& name)
{
    if (name.empty()) {
        napi_env env = JSBind::GetScopedEnv();
        napi_throw_error(env, nullptr, "JSBind: unbindFunction name cannot be empty");
        return -1;
    }
    if (Binding::GetJSFunctionMap().find(name) == Binding::GetJSFunctionMap().end()) {
        napi_env env = JSBind::GetScopedEnv();
        napi_throw_error(env, nullptr, "JSBind: unbindFunction name not found");
        return -1;
    }
                     ...
                     aki::Callback<void (std::string, std::vector<intptr_t>)> outputFunction)
{
    ...
    for (auto& type : Binding::GetBuildInTypeList()) {
        AKI_LOG(ERROR) << "Reflect build in: " << type->name << ", " << type->id;
