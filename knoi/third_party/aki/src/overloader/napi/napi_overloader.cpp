...
napi_value NapiOverloader::Wrapper(napi_env env, napi_callback_info info)
{
    ...
    if (status != napi_ok) {
        napi_throw_error(env, nullptr, "napi_get_cb_info failed in NapiOverloader::Wrapper");
        return nullptr;
    }
    if (bindInfo == nullptr) {
        napi_throw_error(env, nullptr, "bindInfo is nullptr in NapiOverloader::Wrapper");
        return nullptr;
    }
    ...
    if (argc == 2) {
        ...
        AKI_DCHECK(status == napi_ok) << "status(" << status << "): " << GetStatusDesc(status);
        if (status != napi_ok) {
            napi_throw_error(env, nullptr, "napi_typeof failed in NapiOverloader::Wrapper");
            return nullptr;
        }
    ...
    if (argc >= group.methods.size()) {
        napi_throw_type_error(env, nullptr, "No matching overload for argument count");
        return nullptr;
    }

    napi_callback constructorWrapper = reinterpret_cast<napi_callback>(group.methods[argc]);

    if (constructorWrapper == nullptr) {
        napi_throw_type_error(env, nullptr, "No matching overload for argument count");
        return nullptr;
    }
...
napi_value NapiOverloader::CreateValue(napi_env env, napi_callback_info info)
{
    ...
    AKI_DCHECK(status == napi_ok) << "status(" << status << "): " << GetStatusDesc(status);
    if (status != napi_ok) {
        napi_throw_error(env, nullptr, "napi_get_cb_info failed in NapiOverloader::CreateValue");
        return nullptr;
    }
    if (storage == nullptr) {
        napi_throw_error(env, nullptr, "storage is nullptr in NapiOverloader::CreateValue");
        return nullptr;
    }
