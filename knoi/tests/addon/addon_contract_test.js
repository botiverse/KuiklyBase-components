...
assert.equal(addon.setup(validPath, true), undefined);
expectCode(() => addon.setup(validPath, false), 'KNOI_CONFLICTING_SETUP');
...
let receivedWorkerResult = false;
const worker = new Worker(
  `
    'use strict';
    const { parentPort, workerData } = require('node:worker_threads');
    const addon = require(workerData.addonPath);
    addon.setup(workerData.validPath, true);
    addon.init();
    const id = addon.create_function_waiter();
    const payload = JSON.stringify({ worker: '你好🙂' });
    ...
    parentPort.postMessage({
      ...
      payload: addon.wait_on_function_waiter(id),
    });
  `,
  {
    eval: true,
    workerData: { addonPath, validPath },
  },
);
worker.once('message', (message) => {
  ...
  assert.equal(message.payload, JSON.stringify({ worker: '你好🙂' }));
...
worker.once('exit', (code) => {
  ...
  assert.equal(receivedWorkerResult, true, 'worker runtime probe returned no result');
