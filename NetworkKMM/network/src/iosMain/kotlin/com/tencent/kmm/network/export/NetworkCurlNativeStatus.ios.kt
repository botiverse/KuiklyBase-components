...
internal actual fun platformNetworkCurlNativeStatus(): NetworkCurlNativeStatus {
    val linked = IosCurlEngineProvider.nativeLinked
    return NetworkCurlNativeStatus(
        linked = linked,
        http3FeatureAvailable = linked && IosCurlEngineProvider.nativeSupportsHttp3,
        detail = if (linked) null else "iOS curl framework is missing or its wrapper ABI is incompatible."
    )
}
