...
Java_com_tencent_networkkmm_curlspike_MainActivity_runProbe(
    ...
    const bool independent_connections = first.passed && second.passed;

    char output[512];
    std::snprintf(
        output,
        sizeof(output),
        "completed passed=%s independentConnections=%s first={curl=%d http=%ld bytes=%d connectMs=%.3f tlsMs=%.3f totalMs=%.3f} "
        "second={curl=%d http=%ld bytes=%d connectMs=%.3f tlsMs=%.3f totalMs=%.3f}",
        independent_connections ? "true" : "false",
        independent_connections ? "true" : "false",
        first.curl_code,
        first.http_code,
        first.bytes,
